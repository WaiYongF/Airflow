...
def postgres_to_s3(ds_nodash, next_ds_nodash):
    ...
    cursor.execute("""
        SELECT *
        FROM orders
        WHERE date::DATE >= to_date(%s, 'YYYYMMDD')
        AND date::DATE < to_date(%s, 'YYYYMMDD')
    """, (ds_nodash, next_ds_nodash))
    ...
    # cursor.close()
    # conn.close()
    # logging.info("Save orders data into text file: %s", f"dags/get_orders_{ds_nodash}.txt")
...
# Push Orders file to S3 without creating temporary file
'''
def postgres_to_s3(ds_nodash, next_ds_nodash):
    # step 1: query data from postgresql db and save into text file 
    hook = PostgresHook(postgres_conn_id="postgres_localhost")
